    ...
    <Card className="bg-muted flex h-full w-full flex-col rounded-3xl border-none p-4 shadow-none">
      <CardHeader className="flex flex-row items-stretch space-y-0 p-0">
        ...
        <Popover>
          <PopoverTrigger
            className={cn(
              "tablet:hidden bg-background flex aspect-square size-12 items-center justify-center rounded-full border",
            )}
          >
        ...
        <div className="tablet:flex bg-background hidden gap-2 rounded-full p-2">
          {TIMEFRAMES.map((chart) => {
            return (
              <Button
                key={chart}
